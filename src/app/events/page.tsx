...
export default function EventsPage() {
  ...
  const filteredEvents = useMemo(() => {
    return allEvents
      .filter((event) => {
        // Past events filter - when showPastEvents is true, show only upcoming events
        if (showPastEvents) {
          const isEventUpcoming = new Date(event.date.start) > new Date();
          if (!isEventUpcoming) return false;
        }
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-6 max-w-screen-xl">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="flex items-start">
              ...
              >
                {showPastEvents ? "Show All Events" : "Show Upcoming Only"}
              </Button>
            </div>
          </div>
      ...
      {/* All Events - Show all filtered events by default */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredEvents.map((event) => (
                <div key={event.id} className="relative">
                  <EventCard
                    title={formatEventForCard(event).title}
                    description={formatEventForCard(event).description}
                    date={formatEventForCard(event).date}
                    location={formatEventForCard(event).location}
                    type={formatEventForCard(event).type}
                    link={formatEventForCard(event).link}
                    featured={formatEventForCard(event).featured}
                    attendees={formatEventForCard(event).attendees}
                  />
