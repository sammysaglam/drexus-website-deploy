...
export default function AIMarketingPlanPage() {
  ...
  const generatePlan = async () => {
    if (!plan.businessName || !industry || !plan.targetAudience || plan.goals.length === 0) {
    ...
    track("Executed Marketing Tool", {
      location: "/tools/ai-marketing-plan",
      toolName: "AI Marketing Plan Generator",
    ...
    setTimeout(() => {
      const generatedPlan: GeneratedPlan = {
        ...
        marketAnalysis: `The ${industry} industry is experiencing rapid digital transformation. Your target audience (${plan.targetAudience}) increasingly relies on digital channels for research and purchasing decisions. Key competitors (${plan.competitors}) are investing heavily in ${plan.channels[0]}, creating both challenges and opportunities for differentiation through ${plan.uniqueValue}.`,
        ...
        positioningStrategy: `Position ${plan.businessName} as the ${plan.uniqueValue} in the ${industry} space. Emphasize how we uniquely solve the specific challenges faced by ${plan.targetAudience}, unlike ${plan.competitors} who focus on generic solutions.`,

        channelStrategy: plan.channels.slice(0, 4).map((channel, index) => ({
          channel,
          strategy: getChannelStrategy(channel, industry),
