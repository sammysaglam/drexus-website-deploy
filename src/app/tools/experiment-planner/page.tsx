...
import { downloadMarkdown, downloadPDF, getDateString } from "@/lib/download-utils";
...
export default function ExperimentPlannerPage() {
  ...
  const generateMarkdown = () => {
    const markdown = `# ${monthYear} Experiment Plan
**Team:** ${teamName}  
...
`;
  ...
  const generatePDFContent = () => {
    const html = `
      <div class="header">
        <h1>${monthYear} Experiment Plan</h1>
  ...
  const handleDownload = () => {
    if (downloadFormat === "markdown") {
      ...
      const filename = `experiment-plan-${monthYear.toLowerCase().replace(/\s+/g, "-")}-${getDateString()}`;
            ...
            <Card className="mb-8">
              <CardBody className="p-8">
                <h2 className="text-2xl font-semibold text-navy-900 mb-6">
                  {monthYear} Experiments - {teamName}
                </h2>
